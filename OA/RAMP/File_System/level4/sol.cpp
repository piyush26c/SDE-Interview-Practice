...
public:
    ...
    string compressFile(const string& userId, const string& fileName) {
	    if (users.find(userId) == users.end()) {
	        return ""; // User does not exist
	    }
	    ...
	    if (!fileIsOwnedByUser(fileName, userId)) {
	        return "";
	    }

	    // Check if the file is not already compressed
	    // if (fileName.find(".COMPRESSED") != string::npos) {
	    //     return false;
	    // }

	    string compressedFileName = fileName + ".COMPRESSED";
	    TrieNode* compressedNode = findNode(compressedFileName);
	    ...
	    TrieNode* fileNode = findNode(fileName);


	    // Calculate the size of the compressed file (half the original size)
	    int compressedSize = fileNode->size / 2;

	    // Check if the user's capacity allows adding the compressed file
	    if ((user.getUsedSpace() - fileNode->size + compressedSize) <= user.getCapacity()) {
	       	if (deleteNode(fileName)) {
	       		// update the usedCapacity
	       		user.setUsedSpace(user.getUsedSpace() - fileNode->size);
	       		string remainingStorageCapacity = addFileBy(userId, compressedFileName, compressedSize);
    ...
    string decompressFile(const string& userId, const string& fileName) {
	    if (users.find(userId) == users.end()) {
	        return ""; // User does not exist
	    }
	    ...
	    string decompressedFileName = fileName.substr(0, fileName.length() - 11); // Remove ".COMPRESSED";
	    TrieNode* decompressedNode = findNode(fileName);

	    if (decompressedNode != nullptr) {
	    	return "";
	    }


	    TrieNode* decompressedfileNode = findNode(fileName);


	    // Calculate the size of the normal file (double of the original size)
	    int decompressedSize = decompressedfileNode->size * 2;

	    // Check if the user's capacity allows adding the decompressed file
	    if ((user.getUsedSpace() - decompressedfileNode->size + decompressedSize) <= user.getCapacity()) {
	       	if (deleteNode(fileName)) {
	       		// update the usedCapacity
	       		user.setUsedSpace(user.getUsedSpace() - decompressedfileNode->size);
	       		string remainingStorageCapacity = addFileBy(userId, decompressedFileName, decompressedSize);
	            return remainingStorageCapacity;
	       	}
	    }
